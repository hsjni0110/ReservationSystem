package com.reservationsystem.reservation.controller;
...
import com.system.reservationsystem.controller.ReservationController;
...
public class ReservationControllerTest extends ControllerTest {
    ...
    void 예약_가능한_좌석을_조회할_수_있다() throws Exception {
        ...
        // when, then
        mockMvc.perform(post("/reservation/seats")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(availableSeatRequest))
                )
                .andExpect(status().isOk())
                .andExpect(jsonPath("$[0].scheduleSeatId").value(scheduledSeatResponse.getScheduleSeatId()))
                .andExpect(jsonPath("$[0].seatId").value(scheduledSeatResponse.getSeatId()))
                .andExpect(jsonPath("$[0].reserved").value(scheduledSeatResponse.isReserved()));
    }
